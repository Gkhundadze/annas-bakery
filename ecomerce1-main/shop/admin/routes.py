...
def category(): 
    if 'email' not in session:
        flash(f'flease login first','danger')
        return redirect(url_for('login'))
    categoryes = Category.query.order_by(Category.id.desc()).all()
    return render_template('admin/brand.html', title='Category Page', categoryes= categoryes)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Welcome {form.name.data} Thanks you for registring','success')
        return redirect(url_for('login'))
